---
title: "Rihanna Lyrics Sentiment Analysis"
...
# Rihanna Lyrics Sentiment Analysis
...
### Most used words without filtering

```{r topfeatures w/o filtering}
corpus <- corpus(data_clean, text_field = "Lyric")
dfm1 <- dfm(tokens(corpus))
topfeatures(dfm1)
```

### Most used words filtering

```{r topfeatures filtering}
...
### Wordcloud of full discography

```{r wordcloud all albums}
...
```{r words freq analysis}
features_dfm <- textstat_frequency(dfm2, n = 75)
...
#Grouping by Album combining scores
...
#Plot of 5 best scores grouped by album
ggplot(data = head(sum_by_album,5), aes(x = doc_id, y = afinn)) +
  geom_point()
...
#Plot of 5 best scores by song
ggplot(data = head(sum_by_title,5), aes(x = doc_id, y = afinn)) + ylim(0,250)+
  geom_point()
